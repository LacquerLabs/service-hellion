...
ORG = lacquerlabs
NAME = service-hellion
IMAGE = $(ORG)/$(NAME)
VERSION = 0.0.1
PORT_CLIENT_INT = 6004
PORT_CLIENT_EXT = 6004
PORT_STATUS_INT = 6005
PORT_STATUS_EXT = 6005
...
run: ## run it detached
	docker run -p $(PORT_CLIENT_EXT):$(PORT_CLIENT_INT) -p $(PORT_STATUS_EXT):$(PORT_STATUS_INT) --name $(NAME)_run --rm -id $(IMAGE)

runattached: ## run it attached
	docker run -p $(PORT_CLIENT_EXT):$(PORT_CLIENT_INT) -p $(PORT_STATUS_EXT):$(PORT_STATUS_INT) --name $(NAME)_run --rm -it $(IMAGE)

runshell: ## run the container with an interactive shell
	docker run -p $(PORT_CLIENT_EXT):$(PORT_CLIENT_INT) -p $(PORT_STATUS_EXT):$(PORT_STATUS_INT) --name $(NAME)_run --rm -it $(IMAGE) /bin/sh
